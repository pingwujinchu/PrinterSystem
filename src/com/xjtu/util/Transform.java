...
public class Transform {
	...
	private static JSONObject tableToJson(Table table, String parentColumn) {
		...
		jsUserData.put(COLUMN_NAMES, Util.arrayJoin(table.getColumnNames().toArray(), "%s", ","));
		jsUserData.put(COLUMN_TYPES, Util.arrayJoin(table.getColumnTypes().toArray(), "%s", ","));
	...
	private static Object jsonToValue(Object jsValue, DataType type) throws ParseException {
		...
		case TIME:
			return new java.sql.Time(TIME_FORMAT.parse(jsValue.toString()).getTime());
		case DATETIME:
